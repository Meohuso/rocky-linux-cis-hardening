...
CIS 5.3.3.2.3 is Level 1 Server. Its CIS title says **Manual**, while the ComplianceAsCode RHEL 9 control has `status: automated`: the project retains a historical `minclass=4` automation, explaining that all four character classes meet the complexity requirement. The primary mapping is `accounts_password_pam_minclass` with `var_password_pam_minclass=4`. The listed `accounts_password_pam_dcredit`, `accounts_password_pam_lcredit`, `accounts_password_pam_ocredit`, and `accounts_password_pam_ucredit` are related rules only; this module does not configure or combine that separate credit policy. Libpwquality has four character categories and clamps a setting above four to four internally. The module accepts only the meaningful configured domain 0–4: four is compliant, 0–3 are NON_COMPLIANT, and values above four, negative, or malformed are ERROR rather than being presented as stronger policy. It follows the same vendor/administrator drop-in merge by filename and ASCII order, administrator main-file override and vendor fallback, and last-definition precedence as 5.3.3.2.1/2. After `authselect check`, active inline `minclass=` takes precedence. Incorrect inline options require a reviewed authselect profile change; malformed/duplicated options and custom `conf=` paths return ERROR. Remediation changes only `minclass` in the administrator main file, preserves other settings, comments, ownership and permissions, validates after writing, and immediately rolls back a failed validation. Separate rollback state restores/removes only `minclass`, refuses concurrent changes to that property, and preserves `difok` and `minlen`; cross-control tests cover independent rollback. Automated correction of custom `conf=` and inline PAM options remains unsupported. GitHub Actions validation is required before this control is marked complete.

CIS 5.3.3.2.4 is an automated Level 1 Server control mapped to ComplianceAsCode `accounts_password_pam_maxrepeat` with `var_password_pam_maxrepeat=3`. ComplianceAsCode specifies `less than or equal`; its explicit zero guard currently applies to Ubuntu 24.04 only. Libpwquality documents that `maxrepeat=0` disables the consecutive-character check. Therefore this framework accepts only effective values **1–3**; zero, absence and values above three are NON_COMPLIANT. Values one and two remain untouched. Negative values are parsed by libpwquality as integers but are outside a meaningful nonnegative maximum and are rejected as ERROR here rather than presented as stronger policy; malformed values are also ERROR. After `authselect check`, inline PAM `maxrepeat=` takes precedence. Inline corrections require a reviewed authselect profile change; malformed/duplicated options and custom `conf=` paths return ERROR. For default configuration, vendor and administrator `.conf.d` files are merged by name in ASCII order, the administrator file wins a collision, and the administrator main file is read last (with vendor main fallback if absent); the last active setting wins. Remediation writes only `maxrepeat = 3` in the administrator main file and preserves comments, ownership/mode, and `difok`, `minlen`, `minclass`, and all unrelated policy. Its dedicated rollback state restores/removes only the original `maxrepeat` line, retains later independent changes, and refuses to overwrite concurrent administrator changes to `maxrepeat`. Cross-control tests exercise all four controls and independent rollback. Automated correction of custom `conf=` and inline PAM options remains unsupported. GitHub Actions validation is required before this control is marked complete.

CIS 5.3.3.2.5 is an automated Level 1 Server control mapped to ComplianceAsCode `accounts_password_pam_maxsequence` with `var_password_pam_maxsequence=3`. Its template specifies both `less than or equal` and `zero_comparison_operation: greater than`; effective values **1–3** are compliant. Libpwquality uses `maxsequence` for the maximum length of monotonic ascending or descending character sequences (unlike `maxrepeat`, which limits identical consecutive characters); zero disables the sequence check. Absence, zero, and values above three are NON_COMPLIANT, while one and two remain unchanged. Negative integers are outside a meaningful maximum policy and treated as ERROR, as are malformed values. After `authselect check`, active inline PAM `maxsequence=` overrides the default configuration. Inline corrections require a reviewed authselect profile change; malformed/duplicated options and custom `conf=` paths return ERROR. The default configuration merges vendor/administrator `.conf.d` files by name in ASCII order, prefers the administrator file on name collision, and reads the administrator main file last with vendor main fallback if absent; the last active value wins. Remediation writes only `maxsequence = 3` to the administrator main file, preserving comments, ownership/mode, and `difok`, `minlen`, `minclass`, `maxrepeat`, and other properties. Separate rollback state restores/removes only the original `maxsequence` line, preserves later unrelated edits, and refuses concurrent administrator changes to `maxsequence`. Cross-control tests cover all five controls and independent rollback. Automated correction of custom `conf=` and inline PAM options remains unsupported. GitHub Actions validation is required before this control is marked complete.

CIS 5.3.3.2.6 is an automated Level 1 Server control mapped to ComplianceAsCode `accounts_password_pam_dictcheck` with `var_password_pam_dictcheck=1` and the `equals` operation. Libpwquality currently defaults to one and functionally enables dictionary checking for any nonzero value, using cracklib when available. This CIS policy requires an **explicit effective value of exactly one**: absence, zero and other positive integers are NON_COMPLIANT; negative and malformed settings are ERROR under the framework's conservative input-domain convention. After `authselect check`, inline PAM `dictcheck=` takes precedence. Inline corrections require a reviewed authselect profile change; malformed/duplicated options and custom `conf=` paths return ERROR. The standard configuration merges vendor/administrator `.conf.d` files by name in ASCII order, prefers the administrator file on name collision, and reads the administrator main file last with vendor main fallback if absent; the last active value wins. Remediation writes only `dictcheck = 1` to the administrator main file, preserving unrelated comments, ownership/mode, and `difok`, `minlen`, `minclass`, `maxrepeat`, `maxsequence`, and other policy. Dedicated rollback state restores/removes only the original `dictcheck` line, preserves later independent edits, and refuses concurrent administrator changes to `dictcheck`. Cross-control tests cover all six properties and independent rollback. Automated correction of custom `conf=` and inline PAM options remains unsupported. GitHub Actions validation is required before this control is marked complete.

CIS 5.3.3.2.7 is an automated Level 1 Server/Workstation control mapped to ComplianceAsCode `accounts_password_pam_enforce_root` (`xccdf_org.ssgproject.content_rule_accounts_password_pam_enforce_root`), with no numeric tailoring variable. The canonical administrator directive is `enforce_for_root`, off by default. It makes pam_pwquality reject failed quality checks even when root changes the password (old-password comparisons still do not apply to root). Upstream libpwquality `settings.c` declares it `PWQ_TYPE_SET`: a present directive sets the flag to one, even `enforce_for_root=0`, `=1`, or `=anything`; multiple canonical occurrences are harmless. Unlike the preceding six numeric controls there is no last numeric value to compare or override. The framework classifies all active forms with a value or trailing tokens as ERROR because they are misleading or noncanonical, and absence or comments alone as NON_COMPLIANT. Libpwquality merges vendor/admin drop-ins by filename in ASCII order, then reads the admin main file or vendor fallback. Vendor-only presence activates the runtime flag but does not satisfy the explicit administrator configuration requested by the CIS rule's fixtext and audit paths (`/etc/security/pwquality.conf` and its `.d` directory). A canonical admin main or effective admin drop-in suffices; a hidden vendor drop-in does not. `authselect check` runs; canonical inline PAM `enforce_for_root` is allowed but cannot replace the required administrator configuration. Noncanonical inline forms and custom `conf=` paths require review and return ERROR. Apply appends a separate management comment and exact `enforce_for_root` line to the administrator main file, preserving other settings, comments, owner/group/mode; it validates with check and immediately rolls back on failure. Isolated state for 5.3.3.2.7 enables rollback of only the managed pair, refuses edits to that pair, preserves independent changes and removes an initially absent file only when empty. Tests cover interaction and independent rollback with `difok`, `minlen`, `minclass`, `maxrepeat`, `maxsequence`, and `dictcheck`. A custom authselect profile modification and custom pwquality `conf=` remediation are intentionally unsupported; real Rocky Linux 10.2/OpenSCAP comparison remains part of final validation. GitHub Actions validation is required before marking this control complete.
