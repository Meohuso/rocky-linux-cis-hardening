...
setup() {
    ...
    setup_kernel_module_test_environment \
        "1.1.1.8" \
        "usb-storage" \
        "rlch-cis-1.1.1.8-usb-storage.conf"

    RLCH_CIS_1_1_1_8_MODPROBE_DIRECTORY="${RLCH_TEST_MODPROBE_DIRECTORY}"
    RLCH_CIS_1_1_1_8_CONFIGURATION_FILE="${RLCH_TEST_KERNEL_CONFIGURATION_FILE}"
    RLCH_CIS_1_1_1_8_EFFECTIVE_UID="0"

    # shellcheck source=modules/cis/1/1/1/8/module.sh
    source "${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/1/1/1/8/module.sh"
}
...
@test "check succeeds when usb-storage is unavailable" {
...
@test "check succeeds when usb-storage is disabled and unloaded" {
...
@test "check reports non-compliance when usb-storage is loaded" {
    set_kernel_module_available
...
@test "apply creates the managed modprobe configuration" {
    ...
    [ -f "${RLCH_CIS_1_1_1_8_CONFIGURATION_FILE}" ]
...
@test "apply unloads usb-storage when the module is loaded" {
    set_kernel_module_available
    set_kernel_module_loaded
    create_kernel_module_compliant_managed_file
    ...
    [ "${status}" -eq "${RLCH_MODULE_RESULT_CHANGED}" ]
    [ "$(cat "${RLCH_TEST_MODPROBE_REMOVE_LOG}")" = "usb-storage" ]
}

@test "apply fails without root privileges" {
    RLCH_CIS_1_1_1_8_EFFECTIVE_UID="1000"
...
@test "rollback removes the framework-managed configuration" {
    ...
    [ "${status}" -eq "${RLCH_MODULE_RESULT_CHANGED}" ]
    [ ! -e "${RLCH_CIS_1_1_1_8_CONFIGURATION_FILE}" ]
}
...
@test "metadata declares the expected CIS control" {
    ...
    metadata_file="${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/1/1/1/8/metadata.conf"

    clear_module_metadata_variables

    # shellcheck source=modules/cis/1/1/1/8/metadata.conf
    source "${metadata_file}"

    [ "${RLCH_MODULE_ID}" = "1.1.1.8" ]
    [ "${RLCH_MODULE_LEVEL}" = "1" ]
    [ "${RLCH_MODULE_ENABLED}" = "true" ]
    [ "${RLCH_MODULE_REQUIRES_REBOOT}" = "false" ]
    [ "${RLCH_MODULE_OPENSCAP_RULE}" = "xccdf_org.ssgproject.content_rule_kernel_module_usb-storage_disabled" ]
}
