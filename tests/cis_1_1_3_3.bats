...
setup() {
    ...
    RLCH_CIS_1_1_3_3_FSTAB="${RLCH_TEST_FSTAB}"

    source "${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/1/1/3/3/module.sh"
}
...
@test "check succeeds when nosuid is persistent and active on /var" {
...
@test "check rejects a partial nonosuid option match" {
    add_mount_test_fstab_entry "/var" "/dev/mapper/var" "xfs" "defaults,nonosuid"
    add_mount_test_runtime_entry "/var" "/dev/mapper/var" "xfs" "rw,nonosuid,relatime"
...
@test "metadata declares the expected CIS control" {
    ...
    metadata_file="${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/1/1/3/3/metadata.conf"

    clear_module_metadata_variables

    source "${metadata_file}"

    [ "${RLCH_MODULE_ID}" = "1.1.3.3" ]
    [ "${RLCH_MODULE_LEVEL}" = "1" ]
    [ "${RLCH_MODULE_ENABLED}" = "true" ]
    [ "${RLCH_MODULE_REQUIRES_REBOOT}" = "false" ]
    [ "${RLCH_MODULE_OPENSCAP_RULE}" = "xccdf_org.ssgproject.content_rule_mount_option_var_nosuid" ]
}
