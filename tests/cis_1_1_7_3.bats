...
setup() {
    ...
    RLCH_CIS_1_1_7_3_FSTAB="${RLCH_TEST_FSTAB}"

    source "${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/1/1/7/3/module.sh"
}
...
@test "check succeeds when nosuid is persistent and active on /home" {
    add_mount_test_fstab_entry "/home" "/dev/mapper/home" "xfs" "defaults,nosuid"
    add_mount_test_runtime_entry "/home" "/dev/mapper/home" "xfs" "rw,nosuid,relatime"
...
@test "check rejects a partial nonnosuid option match" {
    add_mount_test_fstab_entry "/home" "/dev/mapper/home" "xfs" "defaults,nonnosuid"
    add_mount_test_runtime_entry "/home" "/dev/mapper/home" "xfs" "rw,nonnosuid,relatime"

    run check
    [ "${status}" -eq "${RLCH_MODULE_RESULT_NON_COMPLIANT}" ]
}

@test "check reports non-compliance when only root filesystem has nosuid" {
...
@test "metadata declares the expected CIS control" {
    ...
    metadata_file="${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/1/1/7/3/metadata.conf"

    clear_module_metadata_variables
    source "${metadata_file}"

    [ "${RLCH_MODULE_ID}" = "1.1.7.3" ]
    [ "${RLCH_MODULE_LEVEL}" = "1" ]
    [ "${RLCH_MODULE_ENABLED}" = "true" ]
    [ "${RLCH_MODULE_REQUIRES_REBOOT}" = "false" ]
    [ "${RLCH_MODULE_OPENSCAP_RULE}" = "xccdf_org.ssgproject.content_rule_mount_option_home_nosuid" ]
}
