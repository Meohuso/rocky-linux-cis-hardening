...
setup() {
    ...
    RLCH_CIS_1_1_8_4_FSTAB="${RLCH_TEST_FSTAB}"

    source "${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/1/1/8/4/module.sh"
}
...
@test "check succeeds when nosuid is persistent and active on /dev/shm" {
    add_mount_test_fstab_entry "/dev/shm" "tmpfs" "tmpfs" "defaults,nosuid"
    add_mount_test_runtime_entry "/dev/shm" "tmpfs" "tmpfs" "rw,nosuid,relatime"
...
@test "check rejects a partial nonnosuid option match" {
    add_mount_test_fstab_entry "/dev/shm" "tmpfs" "tmpfs" "defaults,nonnosuid"
    add_mount_test_runtime_entry "/dev/shm" "tmpfs" "tmpfs" "rw,nonnosuid,relatime"

    run check
    [ "${status}" -eq "${RLCH_MODULE_RESULT_NON_COMPLIANT}" ]
}

@test "check reports non-compliance when only /dev has nosuid" {
    add_mount_test_fstab_entry "/dev" "devtmpfs" "devtmpfs" "defaults,nosuid"
    add_mount_test_runtime_entry "/dev" "devtmpfs" "devtmpfs" "rw,nosuid,relatime"
...
@test "metadata declares the expected CIS control" {
    ...
    metadata_file="${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/1/1/8/4/metadata.conf"

    clear_module_metadata_variables
    source "${metadata_file}"

    [ "${RLCH_MODULE_ID}" = "1.1.8.4" ]
    [ "${RLCH_MODULE_LEVEL}" = "1" ]
    [ "${RLCH_MODULE_ENABLED}" = "true" ]
    [ "${RLCH_MODULE_REQUIRES_REBOOT}" = "false" ]
    [ "${RLCH_MODULE_OPENSCAP_RULE}" = "xccdf_org.ssgproject.content_rule_mount_option_dev_shm_nosuid" ]
}
