...
setup() {
    ...
    setup_kernel_module_test_environment \
        "1.1.1.4" \
        "hfsplus" \
        "rlch-cis-1.1.1.4-hfsplus.conf"
    RLCH_CIS_1_1_1_4_MODPROBE_DIRECTORY="${RLCH_TEST_MODPROBE_DIRECTORY}"
...
@test "check succeeds when hfsplus is unavailable" {
...
@test "check succeeds when hfsplus is disabled and unloaded" {
...
@test "check reports non-compliance when hfsplus is loaded" {
    set_kernel_module_available
...
@test "apply creates the managed modprobe configuration" {
    ...
    [ -f "${RLCH_CIS_1_1_1_4_CONFIGURATION_FILE}" ]
    grep -Fqx \
...
@test "apply unloads hfsplus when the module is loaded" {
    set_kernel_module_available
    set_kernel_module_loaded
    create_kernel_module_compliant_managed_file
    ...
    [ "${status}" -eq "${RLCH_MODULE_RESULT_CHANGED}" ]
    [ "$(cat "${RLCH_TEST_MODPROBE_REMOVE_LOG}")" = "hfsplus" ]
}

@test "apply fails without root privileges" {
    RLCH_CIS_1_1_1_4_EFFECTIVE_UID="1000"
...
@test "rollback removes the framework-managed configuration" {
    ...
    run rollback
    [ "${status}" -eq "${RLCH_MODULE_RESULT_CHANGED}" ]
    [ ! -e "${RLCH_CIS_1_1_1_4_CONFIGURATION_FILE}" ]
}
...
@test "metadata declares the expected CIS control" {
    ...
    [ "${RLCH_MODULE_OPENSCAP_RULE}" = "xccdf_org.ssgproject.content_rule_kernel_module_hfsplus_disabled" ]
}
