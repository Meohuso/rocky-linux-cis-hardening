...
setup() {
    ...
    setup_kernel_module_test_environment \
        "1.1.1.3" \
        "hfs" \
        "rlch-cis-1.1.1.3-hfs.conf"
    RLCH_CIS_1_1_1_3_MODPROBE_DIRECTORY="${RLCH_TEST_MODPROBE_DIRECTORY}"
    RLCH_CIS_1_1_1_3_CONFIGURATION_FILE="${RLCH_TEST_KERNEL_CONFIGURATION_FILE}"
    RLCH_CIS_1_1_1_3_EFFECTIVE_UID="0"
...
@test "check succeeds when hfs is unavailable" {
...
@test "check succeeds when hfs is disabled and unloaded" {
...
@test "check reports non-compliance when hfs is loaded" {
    set_kernel_module_available
...
@test "apply creates the managed modprobe configuration" {
    ...
    [ -f "${RLCH_CIS_1_1_1_3_CONFIGURATION_FILE}" ]
    grep -Fqx \
...
@test "apply unloads hfs when the module is loaded" {
    set_kernel_module_available
    set_kernel_module_loaded
    create_kernel_module_compliant_managed_file
    ...
    [ "${status}" -eq "${RLCH_MODULE_RESULT_CHANGED}" ]
    [ "$(cat "${RLCH_TEST_MODPROBE_REMOVE_LOG}")" = "hfs" ]
}

@test "apply fails without root privileges" {
    RLCH_CIS_1_1_1_3_EFFECTIVE_UID="1000"
...
@test "rollback removes the framework-managed configuration" {
    ...
    run rollback
    [ "${status}" -eq "${RLCH_MODULE_RESULT_CHANGED}" ]
    [ ! -e "${RLCH_CIS_1_1_1_3_CONFIGURATION_FILE}" ]
}
...
@test "metadata declares the expected CIS control" {
    ...
    [ "${RLCH_MODULE_ID}" = "1.1.1.3" ]
    [ "${RLCH_MODULE_LEVEL}" = "1" ]
    [ "${RLCH_MODULE_ENABLED}" = "true" ]
    [ "${RLCH_MODULE_REQUIRES_REBOOT}" = "false" ]
    [ "${RLCH_MODULE_OPENSCAP_RULE}" = "xccdf_org.ssgproject.content_rule_kernel_module_hfs_disabled" ]
}
