...
setup() {
    ...
    RLCH_CIS_1_7_5_ISSUE_FILE="${RLCH_TEST_BANNER_ACCESS_ETC}/issue"
    RLCH_CIS_1_7_5_STATE_DIR="${RLCH_TEST_BANNER_ACCESS_STATE}/1.7.5"
    RLCH_CIS_1_7_5_STATE_FILE="${RLCH_CIS_1_7_5_STATE_DIR}/issue.access"

    printf '%s\n' "Authorized users only." > "${RLCH_CIS_1_7_5_ISSUE_FILE}"

    source "${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/1/7/5/module.sh"
}
...
@test "check reports non-compliance when issue is absent" {
    rm -f "${RLCH_CIS_1_7_5_ISSUE_FILE}"
...
@test "apply saves original access metadata" {
    ...
    [ "${status}" -eq "${RLCH_MODULE_RESULT_CHANGED}" ]
    [ "$(cat "${RLCH_CIS_1_7_5_STATE_FILE}")" = "nobody:users:666" ]
}
...
@test "metadata declares the expected CIS control" {
    ...
    source "${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/1/7/5/metadata.conf"

    [ "${RLCH_MODULE_ID}" = "1.7.5" ]
    [ "${RLCH_MODULE_LEVEL}" = "1" ]
    [ "${RLCH_MODULE_ENABLED}" = "true" ]
    [ "${RLCH_MODULE_REQUIRES_REBOOT}" = "false" ]
    [ "${RLCH_MODULE_OPENSCAP_RULE}" = "xccdf_org.ssgproject.content_rule_file_groupowner_etc_issue" ]
}
