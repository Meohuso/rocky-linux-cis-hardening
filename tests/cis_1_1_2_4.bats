...
setup() {
    ...
    RLCH_CIS_1_1_2_4_FSTAB="${RLCH_TEST_FSTAB}"

    # shellcheck source=modules/cis/1/1/2/4/module.sh
    source "${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/1/1/2/4/module.sh"
}
teardown() {
    teardown_mount_test_environment
}

@test "check succeeds when noexec is persistent and active on /tmp" {
    add_mount_test_fstab_entry "/tmp" "/dev/mapper/tmp" "xfs" "defaults,noexec"
    add_mount_test_runtime_entry "/tmp" "/dev/mapper/tmp" "xfs" "rw,noexec,relatime"
...
@test "check rejects a partial nonnoexec option match" {
    add_mount_test_fstab_entry "/tmp" "/dev/mapper/tmp" "xfs" "defaults,nonnoexec"
    add_mount_test_runtime_entry "/tmp" "/dev/mapper/tmp" "xfs" "rw,nonnoexec,relatime"
...
@test "check reports non-compliance when /tmp is not a separate mount" {
    add_mount_test_fstab_entry "/" "/dev/mapper/root" "xfs" "defaults,noexec"
    add_mount_test_runtime_entry "/" "/dev/mapper/root" "xfs" "rw,noexec,relatime"
...
@test "metadata declares the expected CIS control" {
    ...
    metadata_file="${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/1/1/2/4/metadata.conf"

    clear_module_metadata_variables

    # shellcheck source=modules/cis/1/1/2/4/metadata.conf
    source "${metadata_file}"
    [ "${RLCH_MODULE_ID}" = "1.1.2.4" ]
    [ "${RLCH_MODULE_LEVEL}" = "1" ]
    [ "${RLCH_MODULE_ENABLED}" = "true" ]
    [ "${RLCH_MODULE_REQUIRES_REBOOT}" = "false" ]
    [ "${RLCH_MODULE_OPENSCAP_RULE}" = "xccdf_org.ssgproject.content_rule_mount_option_tmp_noexec" ]
}
