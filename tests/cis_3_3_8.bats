...
setup() {
    ...
    export RLCH_CIS_3_3_8_CONFIG="${RLCH_TEST_SYSCTL_CONTROL_CONFIG_DIR}/60-rlch-cis-3.3.8.conf"
    export RLCH_CIS_3_3_8_STATE_DIR="${RLCH_TEST_SYSCTL_CONTROL_ROOT}/state/3.3.8"
    source "${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/3/3/8/module.sh"
    local parameter
    for parameter in "${RLCH_CIS_3_3_8_PARAMETERS[@]}"; do
        sysctl_control_helper_set_runtime "${parameter}" 1
    done
}

cis_3_3_8_write_compliant_config() {
    sysctl_control_helper_write_config "${RLCH_CIS_3_3_8_CONFIG}" <<'EOF'
...
cis_3_3_8_set_compliant_runtime() {
    local parameter
    for parameter in "${RLCH_CIS_3_3_8_PARAMETERS[@]}"; do
...
@test "check requires all runtime and persistent source-route values" {
    cis_3_3_8_write_compliant_config
    cis_3_3_8_set_compliant_runtime
    run check
...
@test "check reports a non-compliant runtime value" {
    cis_3_3_8_write_compliant_config
    sysctl_control_helper_set_runtime net.ipv4.conf.all.accept_source_route 0
    sysctl_control_helper_set_runtime net.ipv4.conf.default.accept_source_route 0
    sysctl_control_helper_set_runtime net.ipv6.conf.all.accept_source_route 0
    run check
...
@test "missing IPv6 parameter is an error rather than not-applicable" {
    grep -v '^net.ipv6.conf.default.accept_source_route=' "${RLCH_TEST_SYSCTL_CONTROL_RUNTIME}" > "${RLCH_TEST_SYSCTL_CONTROL_RUNTIME}.tmp"
    mv "${RLCH_TEST_SYSCTL_CONTROL_RUNTIME}.tmp" "${RLCH_TEST_SYSCTL_CONTROL_RUNTIME}"
    run check
    [ "${status}" -eq "${RLCH_MODULE_RESULT_ERROR}" ]
}

@test "apply disables IPv4 and IPv6 source routing" {
    local result=0 parameter
    apply || result=$?
    [ "${result}" -eq "${RLCH_MODULE_RESULT_CHANGED}" ]
    for parameter in "${RLCH_CIS_3_3_8_PARAMETERS[@]}"; do
        [ "$(sysctl_control_helper_runtime_value "${parameter}")" = 0 ]
        grep -Fqx "${parameter} = 0" "${RLCH_CIS_3_3_8_CONFIG}"
    done
}
...
@test "metadata uses manual mapping for four ComplianceAsCode rules" {
    source "${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/3/3/8/metadata.conf"
    [ "${RLCH_MODULE_ID}" = 3.3.8 ]
    [ "${RLCH_MODULE_LEVEL}" = 1 ]
