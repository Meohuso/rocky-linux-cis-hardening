...
rlch_test_snmp_rpm() {
    if [[ "${1:-}" != "-q" ]] || [[ "${2:-}" != "${RLCH_CIS_2_1_14_PACKAGE}" ]]; then
        return 1
    fi
...
rlch_test_snmp_dnf() {
    ...
    if [[ "${option}" != "-y" ]] || [[ "${package}" != "${RLCH_CIS_2_1_14_PACKAGE}" ]]; then
        return 1
    fi

    case "${action}" in
        remove)
            if [[ "${RLCH_TEST_SNMP_DNF_REMOVE_FAIL}" == "true" ]]; then return 1; fi
            if [[ "${RLCH_TEST_SNMP_KEEP_INSTALLED_AFTER_REMOVE}" != "true" ]]; then
                export RLCH_TEST_SNMP_INSTALLED="false"
            fi
            ;;
        install)
            if [[ "${RLCH_TEST_SNMP_DNF_INSTALL_FAIL}" == "true" ]]; then return 1; fi
