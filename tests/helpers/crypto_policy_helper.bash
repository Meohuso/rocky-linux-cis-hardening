...
case "${1:-}" in
    ...
    --set)
        ...
        if [[ ":${2}:" == *":NO-SSHWEAKMACS:"* ]]; then
            printf '%s\n' 'mac@SSH = HMAC-SHA2-256 HMAC-SHA2-512' >> "${RLCH_TEST_CRYPTO_POLICY_CURRENT_FILE}"
        elif [[ -n "${RLCH_TEST_CRYPTO_POLICY_MACS:-}" ]]; then
            printf 'mac@SSH = %s\n' "${RLCH_TEST_CRYPTO_POLICY_MACS}" >> "${RLCH_TEST_CRYPTO_POLICY_CURRENT_FILE}"
        fi
        if [[ ":${2}:" == *":NO-SHA1:"* ]]; then
...
SCRIPT
    ...
    unset RLCH_TEST_CRYPTO_POLICY_MACS
