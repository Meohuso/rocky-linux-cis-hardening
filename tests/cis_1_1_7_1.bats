...
setup() {
    ...
    RLCH_CIS_1_1_7_1_FSTAB="${RLCH_TEST_FSTAB}"

    source "${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/1/1/7/1/module.sh"
}
...
@test "check succeeds when /home has persistent and runtime mount entries" {
    add_mount_test_fstab_entry "/home"
    add_mount_test_runtime_entry "/home"
...
@test "check reports non-compliance when /home is absent from fstab" {
    add_mount_test_runtime_entry "/home"
...
@test "check reports non-compliance when /home is not mounted at runtime" {
    add_mount_test_fstab_entry "/home"
...
@test "check ignores the root filesystem" {
...
@test "metadata declares the expected CIS control" {
    ...
    metadata_file="${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/1/1/7/1/metadata.conf"

    clear_module_metadata_variables

    source "${metadata_file}"

    [ "${RLCH_MODULE_ID}" = "1.1.7.1" ]
    [ "${RLCH_MODULE_LEVEL}" = "1" ]
    [ "${RLCH_MODULE_ENABLED}" = "true" ]
    [ "${RLCH_MODULE_REQUIRES_REBOOT}" = "false" ]
    [ "${RLCH_MODULE_OPENSCAP_RULE}" = "xccdf_org.ssgproject.content_rule_partition_for_home" ]
}
