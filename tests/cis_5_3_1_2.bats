...
setup() {
    ...
    export RLCH_CIS_5_3_1_2_RPM="${BATS_TEST_TMPDIR}/rpm"
    export RLCH_CIS_5_3_1_2_DNF="${BATS_TEST_TMPDIR}/dnf5"
    cat > "${RLCH_CIS_5_3_1_2_RPM}" <<'EOF'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$RLCH_TEST_RPM_LOG"
exit "${RLCH_TEST_RPM_EXIT:-0}"
EOF
    cat > "${RLCH_CIS_5_3_1_2_DNF}" <<'EOF'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$RLCH_TEST_DNF_LOG"
if [[ " $* " == *' repoquery '* ]]; then
    [[ "${RLCH_TEST_DNF_EXIT:-0}" == 0 ]] || exit "$RLCH_TEST_DNF_EXIT"
    printf '%s\n' "${RLCH_TEST_DNF_AVAILABLE-authselect}"
    exit 0
fi
exit "${RLCH_TEST_DNF_CHECK_EXIT:-0}"
EOF
    chmod +x "${RLCH_CIS_5_3_1_2_RPM}" "${RLCH_CIS_5_3_1_2_DNF}"
    source "${BATS_TEST_DIRNAME}/../modules/cis/5/3/1/2/module.sh"
}

@test "installed current authselect is compliant after a refreshed repository check" {
    run check
    [ "$status" -eq "$RLCH_MODULE_RESULT_SUCCESS" ]
    [ "$(cat "$RLCH_TEST_RPM_LOG")" = '-q authselect' ]
    [ "$(cat "$RLCH_TEST_DNF_LOG")" = $'--refresh --setopt=skip_if_unavailable=False repoquery --available --queryformat=%{name} authselect\n--refresh --setopt=skip_if_unavailable=False check-upgrade authselect' ]
}

@test "absent authselect is noncompliant without querying repositories" {
...
@test "an available authselect update is noncompliant" {
...
@test "no authselect in enabled repositories cannot prove latest version" {
...
@test "apply refuses to install absent authselect automatically" {
    export RLCH_TEST_RPM_EXIT=1
    run apply
    [ "$status" -eq "$RLCH_MODULE_RESULT_ERROR" ]
    [ ! -e "$RLCH_TEST_DNF_LOG" ]
...
@test "metadata is Level 1 and manual because ComplianceAsCode has no exact rule" {
    source "${BATS_TEST_DIRNAME}/../modules/cis/5/3/1/2/metadata.conf"
    [ "$RLCH_MODULE_ID" = 5.3.1.2 ]
    [ "$RLCH_MODULE_LEVEL" = 1 ]
