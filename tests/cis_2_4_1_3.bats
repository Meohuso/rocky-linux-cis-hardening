...
setup() {
    ...
    RLCH_CIS_2_4_1_3_PATH="${RLCH_TEST_CRON_ACCESS_ETC}/cron.hourly"
    RLCH_CIS_2_4_1_3_STATE_DIR="${RLCH_TEST_CRON_ACCESS_STATE}/2.4.1.3"
    RLCH_CIS_2_4_1_3_STATE_FILE="${RLCH_CIS_2_4_1_3_STATE_DIR}/access"
    mkdir -p "${RLCH_CIS_2_4_1_3_PATH}"
    source "${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/2/4/1/3/module.sh"
}
...
@test "check reports non-compliance when cron.hourly is absent" {
    rm -rf "${RLCH_CIS_2_4_1_3_PATH}"
    run check
    [ "${status}" -eq "${RLCH_MODULE_RESULT_NON_COMPLIANT}" ]
}
...
@test "apply records the exact initial access state" {
    ...
    run apply
    [ "${status}" -eq "${RLCH_MODULE_RESULT_CHANGED}" ]
    [ "$(cat "${RLCH_CIS_2_4_1_3_STATE_FILE}")" = "1000:100:775" ]
}
...
@test "metadata declares CIS 2.4.1.3 as a manual composite mapping" {
    clear_module_metadata_variables
    source "${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/2/4/1/3/metadata.conf"
    [ "${RLCH_MODULE_ID}" = "2.4.1.3" ]
    [ "${RLCH_MODULE_LEVEL}" = "1" ]
