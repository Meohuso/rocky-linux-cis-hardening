...
setup() {
    ...
    RLCH_CIS_1_1_2_2_FSTAB="${RLCH_TEST_FSTAB}"

    # shellcheck source=modules/cis/1/1/2/2/module.sh
    source "${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/1/1/2/2/module.sh"
}
...
@test "check succeeds when nodev is persistent and active on /tmp" {
    add_mount_test_fstab_entry "/tmp" "/dev/mapper/tmp" "xfs" "defaults,nodev"
    add_mount_test_runtime_entry "/tmp" "/dev/mapper/tmp" "xfs" "rw,nodev,relatime"
...
@test "check rejects a partial nonodev option match" {
    add_mount_test_fstab_entry "/tmp" "/dev/mapper/tmp" "xfs" "defaults,nonodev"
    add_mount_test_runtime_entry "/tmp" "/dev/mapper/tmp" "xfs" "rw,nonodev,relatime"
...
@test "metadata declares the expected CIS control" {
    ...
    metadata_file="${RLCH_TEST_REPOSITORY_ROOT}/modules/cis/1/1/2/2/metadata.conf"

    clear_module_metadata_variables

    # shellcheck source=modules/cis/1/1/2/2/metadata.conf
    source "${metadata_file}"

    [ "${RLCH_MODULE_ID}" = "1.1.2.2" ]
    [ "${RLCH_MODULE_LEVEL}" = "1" ]
    [ "${RLCH_MODULE_ENABLED}" = "true" ]
    [ "${RLCH_MODULE_REQUIRES_REBOOT}" = "false" ]
    [ "${RLCH_MODULE_OPENSCAP_RULE}" = "xccdf_org.ssgproject.content_rule_mount_option_tmp_nodev" ]
}
